...
SELECT contentsno, title, word
FROM contents
WHERE cateno=9 AND (word LIKE '%SWISS%'); -- 대소문자 구분으로 검색 안됨.

SELECT contentsno, title, word
FROM contents
WHERE cateno=9 AND (UPPER(word) LIKE '%SWISS%'); -- 대소문자를 일치 시켜서 검색

-- 패스워드 확인
SELECT COUNT(*) as cnt 
FROM contents
WHERE contentsno=49 AND passwd='1234';
...
SELECT * FROM contents ORDER BY contentsno DESC;
